...
const EcoTales = async ({ searchParams }) => {
  ...
  if (isEnabled) {
    return (
      <main className="min-h-screen bg-white dark:bg-main overflow-hidden">
        ...
        <EventWrapper page_name={PageEvent.ECO_TALES} />
      </main>
    );
  }
        ...
        <>
          ...
          <div className="w-full">
            <div className="flex mx-auto justify-end max-w-[1560px] py-5 px-[5%] sm:px-[10%] items-center">
              <SearchBar placeholder="Search Eco-Tales" />
            </div>
          </div>

          <ComicList comics={posts.posts} type={"eco-tales"} />
